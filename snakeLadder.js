...
//two players
players = [{name: "P1",position: 0,color: "pink"},{name: "P2",position: 0,color: "yellow"}];
...
for (var y = height; y >= 0; y--) {
  ...
  for (var x = 0; x < width; x++) {
    row.push({x,y,occupied: null,position,color: darkBox ? "#C3FAFF" : "#48D3E0"});
...
drawBoard = () => {
  ...
  ladders.forEach(ladder => {
    ...
    drawLine({ color: isLadder ? "green" : "#FF3333", startPos, endPos });
  });
